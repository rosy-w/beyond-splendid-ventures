 ...
 */
function hello_elementor_child_tour_columns($columns) {
    $columns = array(
        'cb' => $columns['cb'],
        'title' => __('Tour Name', 'hello-elementor-child'),
...
add_filter('manage_tour_posts_columns', 'hello_elementor_child_tour_columns');

/**
 * Display custom column content for Tours
 */
function hello_elementor_child_tour_column_content($column, $post_id) {
    switch ($column) {
        case 'image':
...
add_action('manage_tour_posts_custom_column', 'hello_elementor_child_tour_column_content', 10, 2);
